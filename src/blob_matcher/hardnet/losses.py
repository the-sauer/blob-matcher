...
"""
This module the contains loss functions and utilities related to losses and evaluation.
"""
...
def loss_HardNet_weighted(
    ...
    """
    HardNet margin loss - calculates loss based on distance matrix based on positive distance and closest negative
    distance.
    """
    ...
    if garbage is not None:
        garbage_dist1 = distance_matrix_vector(anchor, garbage)
        garbage_dist2 = distance_matrix_vector(garbage, positive)

    if batch_reduce == 'min':
        min_neg_idx = torch.min(torch.cat([dist_without_min_on_diag, garbage_dist1], dim=1), 1)[1]
        min_neg = torch.min(torch.cat([dist_without_min_on_diag, garbage_dist1], dim=1), 1)[0]
        if anchor_swap:
            min_neg2 = torch.min(torch.cat([dist_without_min_on_diag, garbage_dist2], dim=0), 0)[0]
            min_neg = torch.min(
                min_neg,
                min_neg2
            )
...
def loss_HardNet_weighted_old(anchor,
                          positive,
                          garbage,
                          anchor_swap=False,
                          margin=1.0,
...
if __name__ == "__main__":
    for _ in range(100):
        anchor = torch.rand(5, 128)#.cuda()
        positive = torch.rand(4, 128)#.cuda() 
        garbage = torch.rand(2, 128)#.cuda()

        loss, _ = loss_HardNet_weighted(anchor, positive, garbage, anchor_swap=True, batch_reduce="min")
        print(loss)